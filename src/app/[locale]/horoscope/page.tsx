...
const LABELS: Record<string, Record<string, string>> = {
    title: { ko: '별자리 운세', ja: '星座占い', en: 'Zodiac Horoscope', zh: '星座运势' },
    subtitle: {
        ko: '생년월일을 입력하면 당신의 별자리와 오늘의 운세를 확인할 수 있습니다',
        ja: '生年月日を入力して、あなたの星座と今日の運勢を確認しましょう',
        en: 'Enter your birthdate to discover your zodiac sign and today\'s fortune',
        zh: '输入您的出生日期，查看您的星座和今日运势',
    },
    ...
    const handleAnalyze = () => {
        ...
        interpret({
            ...
            question: question || (loc === 'ko'
                ? `${activeSign.names[loc]} 별자리의 오늘(${today}) 운세를 상세히 분석해주세요. 연애, 재물, 건강, 직장/학업 운을 각각 알려주세요.`
                : loc === 'ja'
                    ? `${activeSign.names[loc]}の今日(${today})の運勢を詳しく分析してください。恋愛、金運、健康、仕事/学業運をそれぞれ教えてください。`
                    : loc === 'zh'
                        ? `请详细分析${activeSign.names[loc]}星座今天(${today})的运势。请分别告诉我爱情、财运、健康、事业/学业运。`
                        : `Please analyze today's (${today}) horoscope for ${activeSign.names[loc]} in detail. Cover love, wealth, health, and career/study fortune.`
            ),
